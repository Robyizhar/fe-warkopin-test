...
import { getData, saveData, detailData, editData } from '../../api/departementAPI';
...
let debouncedRowDepartement = debounce(detailData, 1000);

export const fetchData = () => {
    return  async (dispatch, getState) => {
        dispatch(startFetchingData());

        let perPage = getState().departements.perPage || '';
        let currentPage = getState().departements.currentPage || 1;
        let keyword = getState().departements.keyword || '';
...
export const storeData = () => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            let data = getState().departements.input || [];
...
export const getRow = (id) => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            return await debouncedRowDepartement(id, token);
        } catch (err) {
