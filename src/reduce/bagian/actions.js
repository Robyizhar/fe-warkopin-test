...
import { 
    ...
    SET_STATUS, SET_INPUT, SET_DEPARTEMENT

} from './constants';

import { getData, saveData, detailData, editData } from '../../api/bagianAPI';
...
export const fetchData = () => {
    return  async (dispatch, getState) => {
        ...
        let perPage = getState().bagians.perPage || '';
        let currentPage = getState().bagians.currentPage || 1;
        let keyword = getState().bagians.keyword || '';
...
export const storeData = () => {
    return async (dispatch, getState) => {
        try {
            let token = getState().auth.token || '';
            let data = getState().bagians.input || [];
...
// export const fetchDepartement = () => {
//     return  async (dispatch, getState) => {

//         let token = getState().auth.token || '';
//         const params = {}
//         return await debouncedFetchDepartement(params, token);
//     }
// }
