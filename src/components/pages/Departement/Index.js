...
import { fetchData, setPage, goToNextPage, goToPrevPage, setKeyword, setLimit } from '../../../reduce/departement/actions';
import { deleteData } from '../../../api/departementAPI';
...
const Index = (props) => {
    ...
    if (props.permission.includes(`view departement`) === false)
        redirect(`/not-found`);
    ...
    return (
        <Fragment>
            <div className='row'>
                ...
                {props.permission && props.permission.includes(`add departement`) &&
                    <Link to="/departement/form" className="btn btn-primary float-right" style={{float: 'right'}}>Add&hellip;</Link>}
                </div>
                    ...
                    <div className="card" >
                        <div className="card-body">
                            <table className="table table-bordered">
                                ...
                                {<tbody>
                                    { response.status === 'success' && response.data.data && response.data.data.map((departement, index) => 
                                        <tr key={departement.id}>
                                            <td>{index+1}</td>
                                            <td>{departement.name}</td>
                                            <td>
                                                {props.permission && props.permission.includes(`edit departement`) && 
                                                    <Button onAction={() => editHandler(departement.id)} text={`Edit`} icon={`bi bi-pencil-square`} property={`btn btn-sm btn-light mx-1`}></Button>}
                                                {props.permission && props.permission.includes(`delete departement`) && 
                                                    <Button onAction={() => deleteHandler(departement.id)} text={`Delete`} icon={`bi bi-trash-fill`} property={`btn btn-sm btn-light mx-1`}></Button>}
                                            </td>
                                        </tr>
                                    )}
